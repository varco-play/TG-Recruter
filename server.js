...
// ---------------- EXPRESS ----------------
const app = express();
app.get("/", (req, res) => res.send("🤖 Telegram Recruiting Bot is running."));
...
const T = {
  ...
  es: {
    ...
    askContact:
      "📱 Por favor escriba su número de contacto (con código de país):",
...
function mainMenuKeyboard(lang) {
  return {
    keyboard: [
      ...
      [
        t(lang, "allVacancies"),
        `${t(lang, "chooseLanguageMenu")} ${t(lang, "langFlags")[lang]}`,
      ],
    ],
    resize_keyboard: true,
  };
...
function vacanciesKeyboard(lang) {
  ...
  for (let i = 0; i < VACANCIES.length; i += 2)
    kb.push(VACANCIES.slice(i, i + 2).map((v) => v[lang]));
...
bot.on("message", async (msg) => {
  ...
  if (raw === t(lang, "aboutUs"))
    return bot.sendMessage(chatId, "📖 About us info...", {
      reply_markup: mainMenuKeyboard(lang),
    });
  if (raw === t(lang, "contacts"))
    return bot.sendMessage(chatId, "📞 Contact info...", {
      reply_markup: mainMenuKeyboard(lang),
    });
  if (raw === t(lang, "allVacancies")) {
    s.previousStep = s.step;
  ...
  switch (s.step) {
    ...
    case "ask_experience": {
      if (!t(lang, "expOptions").includes(raw))
        return bot.sendMessage(chatId, t(lang, "invalidOption"), {
          reply_markup: experienceKeyboard(lang),
        });
      s.experience = raw;
